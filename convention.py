...
class Convention(object):
    ...
    def __init__(self, input):
        ...
        #   — governProgramsFraction  --> Доля плавающей части ипотечного покрытия, % от суммы остатков основного долга в ипотечном покрытии
        ...
        # Что такое плавающая часть ипотечного покрытия?
        ...
        # Если Тип расчета купонной выплаты — переменный:
            ...
            # Если ипотечное покрытие стандартного типа, то необходимо задать одно из полей
            ...
            # Если ипотечное покрытие субсидируемого типа, то необходимо задать одно из полей
            ...
            # Если ипотечное покрытие смешанного типа, то необходимо задать два параметра одновременно:
            # Z-СПРЕД (для части выпуска облигаций, которая обеспечивается кредитами без субсидий) и
            # ТРЕБУЕМАЯ НАДБАВКА (для части выпуска облигаций, которая обеспечивается кредитами с субсидиями):
            else:
    ...
    def mbsCashflowModel(self):
        ...
        # Благодаря тому, что модель денежного потока по ипотечному покрытию выстраивает денежный поток отдельно для фиксированной части
        # ипотечного покрытия и отдельно для плавающей части ипотечного покрытия, денежный поток по выпуску облигаций в модельных целях
        # можно разделить на две составные части:
        #       — денежный поток по выпуску облигаций в фиксированной части ипотечного покрытия
        #       — денежный поток по выпуску облигаций в плавающей части ипотечного покрытия
        ...
        #       — fixed — компоненты денежного потока по выпуску ИЦБ ДОМ.РФ в фиксированной
        #       — float — компоненты денежного потока по выпуску ИЦБ ДОМ.РФ в плавающей части
        ...
        #
        # В том случае, если в ипотечном покрытии есть плавающая часть, то также необходимо восстановить все недостающие суммы субсидий.
        ...
        # Непогашенный номинал облигации до Даты первой модельной купонной выплаты в фиксированной части ипотечного покрытия как доля от
        # непогашенного номинала, равная доле фиксированной части на начало расчетного периода Даты первой модельной купонной выплаты
        # (округляется до ближайшей копейки):
        ...
        # Непогашенный номинал облигации до Даты первой модельной купонной выплаты в плавающей части ипотечного покрытия:
        ...
        # облигации выпуска) для частей total (весь выпуск), fixed (фиксированная часть), float (плавающая часть):
        for part in self.mbsModel.keys():
        ...
        # Непогашенный номинал выпуска облигаций до Даты первой модельной купонной выплаты в фиксированной части:
        current_fixed_principal = self.mbsModel['fixed']['principal'] * self.numberOfBonds
        # Непогашенный номинал выпуска облигаций до Даты первой модельной купонной выплаты в плавающей части:
        ...
        # выплаты остаток равен нулю. Указывается отдельно для фиксированной части (fixed) и отдельной для плавающей части (float):
        ...
        while True:
            ...
            # Номинал выпуска облигаций в фиксированной части на начало купонного периода текущей купонной выплаты:
            self.mbsModel['fixed']['issue'].loc[i, 'principalStartPeriod'] = current_fixed_principal

            # Номинал выпуска облигаций в плавающей части на начало купонного периода текущей купонной выплаты:
            ...
            # (available_fixed — по фиксированной части, available_float — по плавающей части):
            ...
            if available_fixed + available_float < current_total_principal:
                ...
                # Направляется на погашение выпуска в фиксированной части:
                issue_amt_fixed = round_floor(available_fixed / num, 2) * num

                # Направляется на погашение выпуска в плавающей части:
                ...
                if current_total_principal >= self.cleanUpRubles and coupon_date < self.calculationRedemptionDate:
                    ...
                    # Частичное погашение выпуска облигаций в плавающей части:
                    ...
                    # Входящий остаток амортизации выпуска облигаций в фиксированной части на следующую купонную выплату:
                    residual_amt_fixed = available_fixed - issue_amt_fixed

                    # Входящий остаток амортизации выпуска облигаций в плавающей части на следующую купонную выплату:
                    ...
                    # Номинал выпуска облигаций в фиксированной части на следующий купонный период:
                    current_fixed_principal -= issue_amt_fixed

                    # Номинал выпуска облигаций в плавающей части на следующий купонный период:
                ...
                # погашения выпуска облигаций для расчета, необходимо произвести полное погашение выпуска облигаций:
                    ...
                    # Полное погашение выпуска облигации в фиксированной части:
                    self.mbsModel['fixed']['issue'].loc[i, 'amortization'] = current_fixed_principal

                    # Погашение выпуска облигаций в плавающей части сверх поступлений по ипотечному покрытию:
            ...
            # Если поступлений за расчетный период с учетом входящего остатка достаточно для погашения выпуска облигаций:
                ...
                # Полное погашение выпуска облигации в плавающей части:
                self.mbsModel['float']['issue'].loc[i, 'amortization'] = current_float_principal
        ...
        # Для частей total (весь выпуск), fixed (фиксированная часть), float (плавающая часть) по остаточному
        ...
        # ----- ПЕРЕМЕННЫЙ ТИП КУПОНА ---------------------------------------------------------------------------------------------------- #
            ...
            # выплаты остаток равен нулю. Указывается отдельно для фиксированной части (fixed) и отдельно для плавающей части (float):
            ...
            for i in range(coupons_number):
                ...
                # Направляется на выплату переменного купона в фиксированной части ипотечного покрытия:
                ...
                # Направляется на выплату переменного купона в плавающей части ипотечного покрытия:
                ...
                # Входящий остаток плавающих сумм в фиксированной части на следующую купонную выплату:
                residual_flt_sum_fixed = available_fixed - bond_coupon_fixed * num

                # Входящий остаток плавающих сумм в плавающей части на следующую купонную выплату:
    ...
    def mbsPricing(self):
        ...
        # В случае переменного купона и наличия в ипотечном покрытии плавающей части разделяем ИЦБ на две части для оценки.
        # ИЦБ в фиксированной части будет оценена по текущей КБД с указанным Z-спредом, а ИЦБ в плавающей части
        # будет оценена путем соотнесения указанной требуемой надбавки с оцененным значением спреда, который генерирует над Ключевой
        # ставкой плавающая часть:
        elif self.couponType == COUPON_TYPE.CHG and not self.poolType == POOL_TYPE.FXD:

            # ИЦБ в фиксированной части:
            ...
            # В случае переменного купона с ипотечным покрытием, состоящим из полностью субсидируемых кредитов, необходимо рассчитать спред
            # к Ключевой ставке, который генерирует ипотечное покрытие в купонах, следующих за датой оценки. В случае переменного купона со
        ...
        if self.calculationType == CALCULATION_TYPE.SET_ZSPRD:
            ...
            # включен в приведенную стоимость ИЦБ в фиксированной части (потому что по построению вся выплата дисконтируется по
            # КБД с Z-спредом). В том случае, если значение следующей после Даты оценки купонной выплаты модельное, то НКД необходимо
            # разделить. НКД по ИЦБ в фиксированной части учтена в приведенной стоимости ИЦБ в фиксированной части, а НКД по ИЦБ в
            # плавающей части необходимо явно рассчитать и добавить в приведенную стоимость:
        ...
        if self.runCashflowModel:
            ...
            for part in pool_parts:
                ...
                if self.poolType in [POOL_TYPE.FLT, POOL_TYPE.MIX] and part in ['total', 'float']:
                    ...
                    # — waKeyRateDeduction — средневзвешенное по остаткам основного долга значение вычета для расчета субсидии по плавающей
                    #                        части на reportDate:
                    subsidy_cf['waKeyRateDeduction'] = base_cf['waKeyRateDeduction'].values

                    # — floatFraction — доля плавающей части ипотечного покрытия на reportDate (в терминах суммы остатка основного долга):
